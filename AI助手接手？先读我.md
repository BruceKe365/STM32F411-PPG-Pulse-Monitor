...
2. `Reports/总体完成进度（2026.6.23定稿）.md`：总进度，最前面的 `2026-06-29 固件参数一致性同步` 是最新状态。
...
| Stress 稳态参数对齐验证 | `training_dataset/reports/full_stress_live_validation_20260629_current/` |
...
| `scripts/validate_stress_hrv_live.py` | 按 MCU 当前参数回放稳态 Stress 窗口和质量门控；不是逐循环调度仿真。 |
| `scripts/validate_mcu_af_live.py` | 复刻 MCU AF live/test 逻辑，含 PPI 质量门控、跳变保持、快慢刷新策略。 |
| `scripts/check_firmware_consistency.py` | 校验固件常量和内嵌权重是否与正式模型、当前报告哈希及文档路径一致。 |
...
| `training_dataset/reports/full_af_validation_20260629_current/` | 按当前固件参数重算的 AF 全量验证报告。 |
| `training_dataset/reports/full_stress_live_validation_20260629_current/` | 按当前固件参数重算的压力稳态窗口与质量门控回放报告；不是逐循环调度仿真。 |
| `training_dataset/reports/full_stress_train_validation_20260629_current/` | 生成当前 MCU 内嵌参数的 WESAD 压力训练/验证报告。 |
...
2026-06-29 固件参数一致性验证结果：

```text
AF local_ppg normal: valid=33/292, median=0%, mean=3.212%, ge80=3.030%
mcu_test_afdb_06995: first_valid=30.244s, risk=85%
public_afdb normal: ge80=0.279%
public_ltafdb normal: ge80=0.685%
public_nsr2db normal: ge80=0.016%
public_afdb af: ge80=36.892%
public_ltafdb af: ge80=45.550%
...
| `scripts/validate_stress_hrv_live.py` | 按 MCU 参数回放稳态 Stress 窗口和质量门控。 |
