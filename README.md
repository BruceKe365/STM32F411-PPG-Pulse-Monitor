...
主屏显示 HR/SpO₂（点按单片机 KEY 可切换显示内容），波形屏显示实时 PPG 波形。
...
- `PCB板文件/脉搏测试仪主板（不带电源）.epro2`
- `PCB板文件/锂电池供电电源板 30mm×40mm.epro2`

对应的 Gerber 制板包也位于 `PCB板文件/`，文件名以 `_Gerber.zip` 结尾，可直接提交板厂制板，便于快速复现项目。
...
| 锂电池供电板和电池 | 可选，见 PCB 工程 | 使用电池供电时参考 `PCB板文件/锂电池供电电源板 30mm×40mm.epro2`。 |

如果使用面包板或飞线复现，所有模块必须共地，并确认 OLED 与 MAX30102 模块均工作在 3.3 V 逻辑电平。更完整的封装、布局和供电连接以 `PCB板文件/` 中的 PCB 工程为准。
...
| `PCB板文件/` | 主板和锂电池供电板的 `.epro2` 工程，以及可直接制板的 `*_Gerber.zip` 压缩包。 |
...
1. 使用 `PCB板文件/` 中与目标板对应的 `*_Gerber.zip` 压缩包制板；如需查看或修改设计，再用嘉立创 EDA 专业版打开同目录下的 `.epro2` 工程，结合上面的引脚表完成主板、供电板和外设连接。
...
除保留其原有许可证的第三方 HAL/CMSIS/USB 库，以及受各自条款约束的外部数据集外，本仓库作者原创的固件、脚本、PCB/Gerber、文档和图片采用 [MIT License](LICENSE)。
